// ============================================
// 🚫 ANTI-BAD WORDS - LWAZI-MD MINI
// 👑 Developer: Lwazi-ᴍᴅ
// 🔥 Auto delete bad words + Warn + Kick
// ============================================

const { LWAZI } = require('../LWAZI');
...
LWAZI({
    ...
    react: "🚫",
    filename: __filename
}, async (LWAZI, mek, m, { from, isGroup, isAdmins, isBotAdmins, isOwner, reply, args, prefix }) => {
...
LWAZI({
    ...
    on: "body",
    filename: __filename
}, async (LWAZI, mek, m, { from, isGroup, isBotAdmins, isAdmins, isOwner, sender, senderNumber, reply }) => {
...
💖 Powered by LWAZI-MD`;
    ...
    if (action === 'kick' && warnCount >= maxWarns) {
        try {
            await LWAZI.groupParticipantsUpdate(from, [sender], 'remove');
            await LWAZI.sendMessage(from, {
                text: `👢 *User kicked!*
...
console.log('🚫 LWAZI-MD - Anti-Bad Words Plugin Loaded! 💖');
