...
const BOT_NAME = config.BOT_NAME || 'LWAZI-MD';
...
function LWAZILog(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [LWAZI-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
...
LWAZILog(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { require(path.join(pluginsDir, file)); }
    catch (e) { LWAZILog(`Failed to load plugin ${file}: ${e.message}`, 'error'); }
}

// ========== EXTRACT MESSAGE BODY (LWAZI-MD Style) ==========
...
// ========== EXTRACT BUTTON ID (LWAZI-MD Style) ==========
...
async function autoFollowChannel(conn, userJid) {
    try {
        ...
        LWAZILog(`[Channel] ${userJid} followed channel`, 'success');
    } catch (e) {
        console.error('[Channel] Follow error:', e.message);
    }
}
// ========== MAIN PAIR FUNCTION ==========
async function LWAZIPair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `LWAZI_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            LWAZILog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                LWAZILog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            fs.ensureDirSync(sessionPath);
            fs.writeFileSync(path.join(sessionPath, 'creds.json'), JSON.stringify(existingSession, null, 2));
            LWAZILog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }
        ...
        if (!conn.authState.creds.registered) {
            LWAZILog(`🔐 Starting NEW pairing process for ${sanitizedNumber}`, 'info');
            try {
                await delay(1500);
                const code = await conn.requestPairingCode(sanitizedNumber);
                LWAZILog(`Pairing Code for ${sanitizedNumber}: ${code}`, 'success');
                if (res && !res.headersSent) {
                    res.send({ code, status: 'new_pairing' });
                }
            } catch (error) {
                LWAZILog(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
            ...
            LWAZILog(`✅ Using existing session for ${sanitizedNumber}`, 'success');
            if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                LWAZILog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });
...
conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
        LWAZILog(`Connected: ${sanitizedNumber}`, 'success');
        ...
        try {
            await LWAZImd(conn);
            LWAZILog(`[System] ✅ Followed all channels`, 'success');
        } catch (e) {
...
*© Powered by ${OWNER_NAME}*`;
    ...
    if (connection === 'close') {
        ...
        if (reason === DisconnectReason.loggedOut) LWAZILog(`Session logged out.`, 'error');
    }
});
        // ========== MESSAGE HANDLER (LWAZI-MD Style) ==========
        ...
        LWAZILog(`LWAZI-MD-MINI Pair error: ${err.message}`, 'error');
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                LWAZILog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            LWAZILog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            LWAZILog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                LWAZILog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
            ...
            if (isNormalError) { LWAZILog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                LWAZILog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                try {
                    ...
                    await LWAZIPair(number, mockRes);
                } catch (e) { LWAZILog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                LWAZILog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
router.get('/force-code', async (req, res) => {
    try {
        ...
        LWAZILog(`🔥 Force pairing requested for ${sanitizedNumber}`, 'warning');
        ...
        if (activeSockets.has(sanitizedNumber)) {
            try {
                ...
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                LWAZILog(`✅ Force disconnected ${sanitizedNumber}`, 'success');
            } catch (error) {
                LWAZILog(`Force disconnect error: ${error.message}`, 'error');
            }
        }
        ...
        if (fs.existsSync(sessionPath)) {
            try {
                await fs.remove(sessionPath);
                LWAZILog(`✅ Deleted local session for ${sanitizedNumber}`, 'success');
            } catch (error) {
                LWAZILog(`Failed to delete local session: ${error.message}`, 'error');
            }
        }
        ...
        try {
            await deleteSessionFromMongoDB(sanitizedNumber);
            await removeNumberFromMongoDB(sanitizedNumber);
            LWAZILog(`✅ Deleted MongoDB session for ${sanitizedNumber}`, 'success');
        } catch (error) {
            LWAZILog(`Failed to delete MongoDB session: ${error.message}`, 'error');
        }

        const lockKey = `LWAZI_lock_${sanitizedNumber}`;
        if (global[lockKey]) {
            global[lockKey] = false;
            LWAZILog(`✅ Cleared lock for ${sanitizedNumber}`, 'success');
        }
        ...
        try {
            ...
            LWAZILog(`🔥 Force pairing code for ${sanitizedNumber}: ${code}`, 'success');

            setTimeout(async () => {
                try {
                    const mockRes = { headersSent: false, send: () => {}, status: () => mockRes, setHeader: () => {}, json: () => {} };
                    await LWAZIPair(sanitizedNumber, mockRes);
                } catch (e) {
                    LWAZILog(`Auto-reconnect after force pairing failed: ${e.message}`, 'error');
                }
            }, 3000);
            ...
            LWAZILog(`Force pairing code generation failed: ${error.message}`, 'error');
        ...
        LWAZILog(`Force pairing error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/force-reset', async (req, res) => {
    try {
        ...
        LWAZILog(`🔥🔥 FORCE RESET requested for ${sanitizedNumber}`, 'warning');
        ...
        if (activeSockets.has(sanitizedNumber)) {
            try {
                ...
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                LWAZILog(`✅ Disconnected ${sanitizedNumber}`, 'success');
            } catch (error) {
                LWAZILog(`Disconnect error: ${error.message}`, 'error');
            }
        }
        ...
        if (fs.existsSync(sessionPath)) {
            try {
                await fs.remove(sessionPath);
                LWAZILog(`✅ Deleted local session`, 'success');
            } catch (error) {
                LWAZILog(`Failed to delete local session: ${error.message}`, 'error');
            }
        }
        ...
        try {
            ...
            LWAZILog(`✅ Deleted all MongoDB data`, 'success');
        } catch (error) {
            LWAZILog(`Failed to delete MongoDB data: ${error.message}`, 'error');
        }
        ...
        LWAZILog(`✅ Force reset completed for ${sanitizedNumber}`, 'success');
        ...
        LWAZILog(`Force reset error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/check-session', async (req, res) => {
    try {
        ...
        LWAZILog(`Check session error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/pair-status', async (req, res) => {
    try {
        ...
        LWAZILog(`Pair status error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/code', async (req, res) => {
    ...
    await LWAZIPair(req.query.number, res);
});
...
router.get('/react', async (req, res) => {
    try {
        ...
        if (!postId) {
            try {
                ...
                if (result && result.messages && result.messages.length > 0) {
                    ...
                    LWAZILog(`Auto-detected post ID: ${postId}`, 'success');
                } else {
                    return res.status(400).json({
                        status: 'error',
        ...
        LWAZILog(`React error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/vote', async (req, res) => {
    try {
        ...
        LWAZILog(`Vote error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/react-vote', async (req, res) => {
    try {
        ...
        LWAZILog(`React-vote error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
            message: 'Failed',
            error: error.message
...
router.get('/users', async (req, res) => {
    try {
        ...
        LWAZILog(`Users error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
async function autoReconnectFromMongoDB() {
    try {
        LWAZILog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { LWAZILog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
            if (!activeSockets.has(number)) {
                const mockRes = { headersSent: false, json: () => {}, status: () => mockRes };
                await LWAZIPair(number, mockRes);
        ...
        LWAZILog('Auto-reconnect completed', 'success');
    } catch (e) {
        LWAZILog(`autoReconnectFromMongoDB error: ${e.message}`, 'error');
    }
}
...
process.on('uncaughtException', (err) => {
    LWAZILog(`Uncaught exception: ${err.message}`, 'error');
});
